...
This project has two raw data files at different scales from a study of infants, children, and adults watching a series of 7 video clips. I wrote Steps 1 and 2 to import and merge the data, and kept them here for your reference.
...
Each question about watching each video is a column, so pivot_longer. Then, use separate to get just the video name into it's own column.
...
```{r, include=TRUE, cache=TRUE, results='markup'}
ds %>% group_by(id, age_group) %>% 
  summarize(precision = mean(precision, na.RM = T)) %>% 
  filter(precision > 2.5) %>% kbl() %>% kable_styling(bootstrap_options = c("striped", "hover"))
```

```{r, include=TRUE, cache=TRUE}
ds %>% filter(precision > 2.5) %>% kbl() %>% kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r, include=TRUE, cache=TRUE}
ds %>% group_by(age_group) %>% summarize(across(precision, list(M = mean, MIN = min, MAX = max))) %>% kbl() %>% kable_styling()
```
